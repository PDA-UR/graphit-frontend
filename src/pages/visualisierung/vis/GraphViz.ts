...
/* ---- NOTES ---- 
// Adjust the visibility of Nodes on different zoom levels -> instead of scroll use +/-
    // zoom in = ++ visible degrees, 
    // Zoomlevel = amount of visible nodes, by degree
    // Every zoomlevel coresponds to one degree-level
    private adjustGraph = (zoomLevel:any, mousePos:any) => {
        //console.log("e", e);
        //console.log("mousepos", mousePos);
        // Get mouse-position -> witch nodes are closest -> reveal their neighbours/connections

        
        if(this.oldZoom > zoomLevel) { // zoom out
            this.visibleDegree += 2;
            console.log("zoom out", this.visibleDegree);
            this.cy.nodes("[[degree < " + this.visibleDegree + "]]").addClass("hide");
        } else {
            // all get connected nodes of the visible nodes and remove hide
            this.visibleDegree -= 2;
            console.log("zoom in", this.visibleDegree);
            this.cy.nodes("[[degree >" + this.visibleDegree + "]]").removeClass("hide");
        }

        this.oldZoom = zoomLevel;
        this.cy.resize();
    }
    // TODO: Graph reveals only elements that are close/connected to node, that you scroll into
    */

    // Trys for show connected()
    /*
    /*const connected = this.getConnected(target);
        console.log(connected[4].style().opacity == 1);
        const expanded = connected[4].style().opacity == 1;

        if(expanded) {
            console.log("collapse node");
            this.cy.elements().not(connected).filter("[[degree >"+ this.visibleDegree + "]]").removeClass("ghost");
            //const hide = connected.filter("[[degree <"+ this.visibleDegree + "]]");
            //hide.nodes().addClass("ghost");
            //hide.connectedEdges().addClass("hide-edges");
            //fit graph into window!!
            this.cy.elements().layout(layoutOptions.fcose);
            this.willExpand = true;
        } else {
            console.log("expand node");
            this.cy.elements().not(connected).addClass("ghost"); // visible degree?
            //this.cy.elements().not(connected).edges().addClass("hide-edges");
            connected.nodes().removeClass("ghost");
            connected.edges().removeClass("hide-edges");
            connected.layout(layoutOptions.fcose).run();
        ...
        const shown = connected[4].style("opacity"); // check if the nodes are shown already

        if(shown == 1) {
            console.log("close nodes connected to ", target.id());
            const hide = connected.filter("[[degree <"+ this.visibleDegree + "]]");
            hide.nodes().addClass("ghost");
            hide.connectedEdges().addClass("hide-edges");
        } else {
            console.log("open nodes connected to ", target.id());
            connected.nodes().removeClass("ghost");
            connected.edges().removeClass("hide-edges");
            // Adjust the layout for the opened nodes
            connected.layout(layoutOptions.fcose).run();
        }*/